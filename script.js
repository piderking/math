document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("contextmenu", (event) => {
    ...
    if (equationElement) {
      ...
      const latex = typst2tex(typ);

      if (latex || typ) {
        // Only show menu if at least one data attribute is present
        ...
        customMenu.querySelector(".copy-tex").style.display = latex
          ? "block"
          : "none";
        customMenu.querySelector(".copy-typ").style.display = typ
          ? "block"
          : "none";
...
document.addEventListener("DOMContentLoaded", (doc, event) => {
  const equations = document.getElementsByClassName("graph");

  for (let el of equations) {
    console.log("Loading Calculator");
    var calculator = Desmos.GraphingCalculator(el);

    for (let i = 0; i < el.children.length; i++) {
      let child = el.children[i];
      if (child.hasAttribute("equation")) {
        calculator.setExpression({
          id: `equation-${i}`, // Unique ID for this expression
          latex: typst2tex(child.getAttribute("equation")), // The equation in LaTeX format
        });
      } else if (child.hasAttribute("point")) {
        calculator.setExpression({
          id: `point-${i}`, // Unique ID for this expression
          latex: typst2tex(child.getAttribute("point")), // The equation in LaTeX format
        });
      }
    }
